...
function getRecentCalls($logPath, $minutes = null, $serverLat = null, $serverLon = null, $sourceFilter = '') {
	...
	foreach (array_reverse($lines) as $line) {
		if (preg_match('/^(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})\.\d+\s+(\S+)\s+([RT])\s+([A-Z0-9\-]+)>/', $line, $m)) {
			...
			if (
				preg_match('/([!:@])(?:\d{6}[hz])?([0-9]{4,5}\.\d{2}[NS])[^0-9A-Z]?([0-9]{5}\.\d{2}[EW])/', $line, $coord)
			 || preg_match('/!([0-9]{4,5}\.\d{2}[NS])[A-Z]?([0-9]{5}\.\d{2}[EW])/', $line, $coord)
			) {
				$latStr = $coord[1];
				$lonStr = $coord[2];
...
function extractAprsMessage($line) {
	...
	// Skip full-line structured position/weather headers
	if (preg_match('/^([!@=;T#])[^:]/', $payload)) return null;

	// Handle message to specific destination
	if (str_starts_with($payload, '::') && strlen($payload) > 10) {
		$payload = substr($payload, 10); // drop "::DEST    :" (9 chars + delimiter)
	}

	// Remove Mic-E or telemetry-looking suffixes like |"M%C|" or |junk|
	if (preg_match('/(.+?)\\|["a-zA-Z0-9%]{3,}\\|"?$/', $payload, $cm)) {
		$payload = trim($cm[1]);
	}

	// Final fallback: must contain 4+ readable ASCII characters
	if (preg_match('/([a-zA-Z].{3,})/', $payload, $cm)) {
		return trim($cm[1]);
	}
